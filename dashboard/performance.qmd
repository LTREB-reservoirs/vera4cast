...
## just use minimum date from full list of max_reference_datetimes
recent_forecast_submission_date <- min(reference_datetimes$reference_datetime_max)
...
df <- duckdbfs::open_dataset(paste0("s3://",config$forecasts_bucket,
  ...
  filter(reference_datetime == recent_forecast_submission_date) |> 
...
reference_date <- lubridate::as_date(recent_forecast_submission_date)
```
...
df |> 
  ...
  forecast_plots_mean(ylabel = 'Chla (ugL)')
}
```
...
df |> 
  filter(variable == c("Temp_C_mean"), 
         site_id != 'tubr')  |> 
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots_mean(ylabel = 'Temp (degC)')
```
...
df |> 
  filter(variable == c("DO_mgL_mean"),
         site_id != 'tubr') |> 
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots_mean(ylabel = 'DO (mgL)')
}
```
...
df |> 
  filter(variable == c("fDOM_QSU_mean"))  |> 
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots_mean(ylabel = 'fDOM (QSU)')
}
```
...
df |> 
  filter(variable == c("Secchi_m_sample")) |>
    ...
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots_mean(ylabel = 'Secchi Depth (m)')
...
df |> 
  filter(variable == c("CH4_umolL_sample"),
         site_id != 'tubr')  |> 
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots_mean(ylabel = 'CH4 (umolL)')
}
```
...
df |> 
  filter(variable == c("CH4flux_umolm2s_mean"))  |> 
    ...
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots_mean(ylabel = 'CH4 Flux (umolm2s)')
}
```
...
df |> 
  filter(variable == c("CO2flux_umolm2s_mean"))  |> 
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots_mean(ylabel = 'CO2 Flux (umolm2s)')
}
```
### Inflow discharge
...
df |> 
  filter(variable == c("Flow_cms_mean"))  |> 
    ...
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots_mean(ylabel = 'Flow (cms)')
}
```
:::
Click [here](performance_variables.qmd) to view the baseline forecasts with uncertainty for the complete list of variables  
...
df_scores <- duckdbfs::open_dataset(paste0("s3://",config$scores_bucket,
                                                     "/bundled-parquet/"),
                                              s3_endpoint = "amnh1.osn.mghpcc.org",
                                              anonymous = TRUE) |> 
...
model_active_cutoff_date <- Sys.Date() - lubridate::years(1)

## find models with at least one submission in the last year
active_models_time_constraint <- df_scores |> 
  filter(reference_datetime > model_active_cutoff_date) |> 
  group_by(model_id) |> 
  summarise(submit_count = n_distinct(reference_datetime)) |> 
  ungroup() |> 
  pull(model_id)

## find models with at least 10 submissions
active_model_submit_constraint <- df_scores |> 
  group_by(model_id) |> 
  summarise(submit_count = n_distinct(reference_datetime)) |> 
  ungroup() |> 
  filter(submit_count >= 10) |> 
  pull(model_id)

## filter scores to show only "active" models
df <- df_scores |> 
  filter(model_id %in% active_models_time_constraint & 
           model_id %in% active_model_submit_constraint)
