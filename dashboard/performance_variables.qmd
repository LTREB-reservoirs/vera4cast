...
## Surface Depth Model Forecasts 
Forecasts shown at each reservoir's respective focal depth (found [here](https://www.ltreb-reservoirs.org/vera4cast/targets.html#target-variable-list))


## Physical Variables (surface)
...
df |> 
  filter(variable == c("SpCond_uScm_mean"), 
         site_id != 'tubr')  |> 
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots(ylabel = 'Cond (uScm)')
}
```
...
df |> 
  filter(variable == c("Turbidity_FNU_mean"), 
         site_id != 'tubr')  |> 
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots(ylabel = 'Turbidity (FNU)')
}
```
...
df |> 
  filter(variable == c("SchmidtStability_Jm2_mean"), 
         site_id != 'tubr')  |> 
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots(ylabel = 'Stability (Jm2)')
}
```
:::
...
df |> 
  filter(variable == c("GreenAlgae_ugL_sample"), 
         site_id != 'tubr')  |> 
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots(ylabel = 'Algae (ugL)')
}
```
...
df |> 
  filter(variable == c("TN_ugL_sample"), 
         site_id != 'tubr')  |> 
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots(ylabel = 'Nitrogen (ugL)')
}
```
...
df |> 
  filter(variable == c("TP_ugL_sample"), 
         site_id != 'tubr')  |> 
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots(ylabel = 'Phosphorus (ugL)')
}
```
...
df |> 
  filter(variable == c("CO2_umolL_sample"), 
         site_id != 'tubr')  |> 
    mutate(observation = as.numeric(NA)) |> 
  forecast_plots(ylabel = 'CO2 (umolL)')
}
```
:::
...
#          site_id != 'tubr')  |> 
#     mutate(observation = as.numeric(NA)) |> 
#   forecast_plots(ylabel = 'DRSi (mgL)')
# }
```
...
```{r}
df_deep <- duckdbfs::open_dataset(paste0("s3://",config$forecasts_bucket,
                                                     "/bundled-summaries/project_id=",
                                                     config$project_id, "/duration=P1D"),
                                              s3_endpoint = "amnh1.osn.mghpcc.org",
                                              anonymous = TRUE) |> 
  ...
  left_join(sites, by = "site_id", copy = TRUE) |> 
  filter(site_id %in% sites$site_id) |>
  filter((depth_m == 9 & site_id == "fcre") | (depth_m == 8 & site_id == "bvre"), 
         datetime > reference_datetime) |> 
  collect()
```

## Bottom Depth Model Forecasts 
Forecasts shown at one meter off the bottom for each reservoir 
